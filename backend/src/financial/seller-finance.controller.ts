...
import { SellerFinanceActivityQueryDto, SellerFinanceSummaryQueryDto } from './seller-finance.dto';
...
export class SellerFinanceController {
  ...
  @Get('summary')
  summary(@CurrentUser() user: { userId: string }, @Query() query: SellerFinanceSummaryQueryDto) {
    void query;
    return this.service.summary(user.userId);
  }
  @Get('activity')
  activity(@CurrentUser() user: { userId: string }, @Query() query: SellerFinanceActivityQueryDto) {
