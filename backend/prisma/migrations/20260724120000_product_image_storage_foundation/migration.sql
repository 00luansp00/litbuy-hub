...
CREATE TABLE "ProductImage" (
  ...
  "uploadedAt" TIMESTAMP(3), "uploadExpiresAt" TIMESTAMP(3) NOT NULL, "deletedAt" TIMESTAMP(3),
  ...
  CONSTRAINT "ProductImage_lifecycle_consistent" CHECK (
    ("status" = 'PENDING_UPLOAD' AND "uploadedAt" IS NULL AND "deletedAt" IS NULL AND "isCover" = false) OR
    ("status" = 'READY' AND "uploadedAt" IS NOT NULL AND "deletedAt" IS NULL) OR
    ("status" = 'DELETED' AND "deletedAt" IS NOT NULL AND "isCover" = false)
  ),
...
CREATE INDEX "ProductImage_productId_status_uploadExpiresAt_idx" ON "ProductImage"("productId", "status", "uploadExpiresAt");
