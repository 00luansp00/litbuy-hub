-- K keeps FinancialHold.releaseEligibleAt as the immutable base deadline. Move the
-- cross-row effective-boundary rule from the row-local CHECK into the lifecycle guard.
  ...
  IF TG_OP = 'UPDATE' AND OLD."reason" = 'DELIVERY_PROTECTION' THEN
    ...
    IF OLD."status" = 'RELEASE_ELIGIBLE' AND NEW."status" = 'RELEASED' THEN
      ...
      IF release_posting."id" IS NULL
        ...
        OR release_posting."referenceId" <> NEW."id"::text
...
ALTER TABLE "FinancialHold"
  VALIDATE CONSTRAINT "FinancialHold_delivery_protection_valid_check";
