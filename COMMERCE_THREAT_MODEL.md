...
| Pagamento confirmado após expiração    | entrega sem estoque ou perda | não reativar pedido/não reconsumir reserva; incidente | reconciliação de eventos tardios | webhook pago após TTL e reserva liberada    |
| Over-refund concorrente                | devolução acima do capturado | limite agregado e reserva transacional                | soma de refunds versus captura   | duas solicitações simultâneas excedentes    |
| Refunds parciais simultâneos           | estado/valor divergente      | lock/update condicional agregado                      | comparar registros e agregado    | dois refunds dentro e fora do limite        |
