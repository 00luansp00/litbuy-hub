...
| Buyer — carrinho no navegador      | **CONCLUÍDO**                              | #57–#61               | Cliente HTTP real, cache React Query, adição pela página pública real, `/carrinho` multi-seller real com leitura, update, remoção e paginação, e navegação global sem estado mock. Superfícies legacy fora do Alpha não escrevem mais no carrinho mock. `CartProvider`/`cartService` remanescentes pertencem exclusivamente ao checkout ainda pendente. |
