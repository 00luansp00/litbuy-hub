...
|---|---|---|---|---|---|---|---|---|---:|---|---|
| COMMERCE-1SKU | Uma linha vendável por Order | Freeze; commerce docs | `CartItem`/`OrderItem` persistem; cart/checkout backend existem, mas schema e services admitem múltiplas linhas | Um SKU/variant por Order; qty N do mesmo SKU; FIXED qty=1; QUOTE sem checkout; backend authority | Restringir cart, checkout, fingerprint e invariants | — | AI_CANDIDATE | C | 0 | PARTIAL | IMPLEMENTED |
| RELEASE-POLICY-HIERARCHY | SUBCATEGORY > CATEGORY > DEFAULT | Freeze | `SellerReleasePolicyVersion/Rule` e resolver existem, porém baseline corrente é por classificação/COMPLETED e não prova hierarquia target | IDs estáveis, versão, fallback 7d, fail closed | Novo modelo/resolver e reconciliação do baseline | COMMERCE-1SKU | AI_CANDIDATE | D | 1 | PARTIAL | IMPLEMENTED |
| RELEASE-CHECKOUT-SNAPSHOT | Snapshot da policy | Freeze | `FinancialHold` guarda policy/rule/delay aplicado; aplicação ocorre no fluxo corrente pós-COMPLETED, não no checkout target | Congelar source/rule/version/base delay/classificação no checkout; não retroativo | Mover/estender snapshot para Order no checkout | hierarchy | AI_CANDIDATE | E | 1 | PARTIAL | IMPLEMENTED |
| RELEASE-DELIVERY-CLOCK | Relógio financeiro na entrega | Freeze | Fulfillment persiste delivery/`deliveredAt`, mas hold corrente é criado/aplicado condicionado a `COMPLETED` | timestamp server de entrega; nunca compra, COMPLETED ou confirmação Buyer | Separar evento entregue de conclusão/confirm | snapshot | AI_CANDIDATE | F | 1 | PARTIAL | IMPLEMENTED |
| RELEASE-ELIGIBILITY | Cálculo e blockers | Freeze; Recovery | `releaseEligibleAt` e serviço de eligibility existem; disputa/blockers target ausentes | `baseReleaseEligibleAt`, disputa prevalece, seller-win conforme contrato | Novo relógio, blockers e semântica de decisão | clock; dispute core | HYBRID_REVIEW_REQUIRED | G1 | 1 | PARTIAL | IMPLEMENTED |
| RELEASE-EXECUTION | HELD → AVAILABLE | Freeze; financial docs | Serviço idempotente/ledger de release existe, preso ao contrato corrente | Executar somente elegível; separar release de payout/withdrawal | Adaptar preconditions sem quebrar ledger/idempotência | eligibility | HYBRID_REVIEW_REQUIRED | G2 | 1 | PARTIAL | IMPLEMENTED |
| SELLER-MAX-LISTING | Opt-in por anúncio e snapshot | Freeze | `requestedSellerPlan=LIT_MAX` em ListingDraft e dimensão de fee existem; benefício completo/snapshot não provados | Opt-in listing, snapshot checkout, não assinatura | Materialização e snapshot autoritativos | 1SKU; tiers | AI_CANDIDATE | I1 | 2 | PARTIAL | IMPLEMENTED |
| SELLER-MAX-FEE | Fee Seller 2,99% na venda | Freeze | Fee engine é genérico; nenhuma policy target 2,99% provada | Seller-only, sale-only, coexiste com tier | Policy/version/snapshot/posting/reversal | MAX listing; fee snapshot; refund contract | HYBRID_REVIEW_REQUIRED | I2 | 2 | PARTIAL | IMPLEMENTED |
...
| FEE-REFUND-REVERSAL | Reversão total/proporcional | Freeze | UNKNOWN/NOT_PROVEN para engine final | Reverter somente fee própria proporcional; PSP separado | Contrato de refund obrigatório antes do engine | refund contract; fee snapshot | HUMAN_PROD_REVIEW | B→W2 | 0 | NOT_IMPLEMENTED | HUMAN_PROD_REVIEW |
...
| LITPOINTS-REFUND | Reversão/devolução LP | Freeze | NOT_PROVEN | reversão proporcional; LP volta com novos 3 meses; sem novo earn | Refund contract e idempotência | LP ledger; refund engine | HUMAN_PROD_REVIEW | W | 7 | NOT_IMPLEMENTED | HUMAN_PROD_REVIEW |
...
| WITHDRAWAL-STANDARD | 60h, R$0 | Freeze | Withdrawal schema/policy foundation usa baseline antigo | 60h corridas, zero | Versioned policy/orchestration | ledger; blockers | HYBRID_REVIEW_REQUIRED | AG1 | 6 | PARTIAL | IMPLEMENTED |
| WITHDRAWAL-EXPRESS | 12h, R$10 | Freeze | Current enum/policy não prova EXPRESS target | 12h corridas, R$10, não instantâneo | Policy/snapshot/posting | standard; blockers | HYBRID_REVIEW_REQUIRED | AG2 | 6 | NOT_IMPLEMENTED | IMPLEMENTED |
| WITHDRAWAL-INSTANT | Ausência no target | Freeze | `INSTANT` existe no schema/teste current | Não existe/não aparece ativo | Deprecar sem reescrever histórico | withdrawal migration | AI_CANDIDATE | AG3 | 5 | SUPERSEDED | SUPERSEDED |
...
| DISPUTE-POST-RELEASE-RECOVERY | Recovery após release | Recovery | NOT_PROVEN | pode gerar recovery/deficit; sem instant refund promise | Final decision + resources | refund contract; recovery core | HUMAN_PROD_REVIEW | AA | 7 | NOT_IMPLEMENTED | HUMAN_PROD_REVIEW |
...
| REFUND-DELIVERY-GUARANTEE-CONTRACT | Contrato anterior ao engine | Freeze; Recovery | Requisito identificado; contrato específico completo ainda ausente | Definir total/partial, fees, PSP, method, MAX, tier, VIP, LP, responsabilidade, chargeback | DOCS PR antes de engine | — | HYBRID_REVIEW_REQUIRED | B | 0 | DOCS_ONLY | DOCS_ONLY |
...
Há **57 capabilities** nesta versão: `IMPLEMENTED 0`, `PARTIAL 19`, `NOT_IMPLEMENTED 25`, `DOCS_ONLY 1`, `DESIGN_REQUIRED 9`, `HUMAN_PROD_REVIEW 1`, `DEFERRED 1`, `SUPERSEDED 1`. A contagem usa **Current status**; target humano não altera a evidência CURRENT.
...
1. Release CURRENT usa `COMPLETED`/policy corrente; target começa em `deliveredAt` e dispensa rating/confirmation normal. Requer D–G; evidência histórica permanece intacta.
...
3. Withdrawal CURRENT contém `INSTANT`/baseline antigo; target tem somente STANDARD 60h e EXPRESS 12h/R$10. Requer depreciação compatível, sem apagar histórico.
