...
| RELEASE-POLICY-HIERARCHY | SUBCATEGORY > CATEGORY > DEFAULT | Freeze | `SellerReleasePolicyVersion/Rule` e resolver existem; CURRENT é GLOBAL ONLY via a única regra enabled `DELIVERY_PROTECTION_DEFAULT`, sem qualifier de category/subcategory/classification/plan/product/risk/KYC/reputation. Separadamente, o hold atual é processado após `COMPLETED`/PAID/CONFIRMED | IDs estáveis, versão, fallback 7d, fail closed | Implementar hierarquia e qualifiers estáveis sem confundir policy resolution com o lifecycle gate do hold | COMMERCE-1SKU | AI_CANDIDATE | D | 1 | PARTIAL | IMPLEMENTED |
| RELEASE-CHECKOUT-SNAPSHOT | Snapshot da policy | Freeze | `FinancialHold` guarda version/rule/delay/application timestamp/`releaseEligibleAt`; o snapshot global CURRENT ocorre na criação/processamento do hold, cujo fluxo exige `COMPLETED`/PAID/CONFIRMED, e não no checkout | Congelar source/rule/version/base delay/classificação no checkout; não retroativo | Mover/estender para snapshot hierárquico no Order durante checkout | hierarchy | AI_CANDIDATE | E | 1 | PARTIAL | IMPLEMENTED |
...
Há **57 capabilities** nesta versão: `IMPLEMENTED 0`, `PARTIAL 20`, `NOT_IMPLEMENTED 25`, `DOCS_ONLY 1`, `DESIGN_REQUIRED 9`, `HUMAN_PROD_REVIEW 1`, `DEFERRED 1`, `SUPERSEDED 0`. A contagem usa **Current status**; target humano não altera a evidência CURRENT.
...
1. Release policy CURRENT é GLOBAL ONLY via `DELIVERY_PROTECTION_DEFAULT`, sem hierarchy/qualifiers; separadamente, o lifecycle CURRENT do hold exige `COMPLETED`/PAID/CONFIRMED. O target adiciona SUBCATEGORY > CATEGORY > DEFAULT, snapshot no checkout e relógio em `deliveredAt`, dispensando rating/confirmation como condição normal. Requer D–G; evidência histórica permanece intacta.
