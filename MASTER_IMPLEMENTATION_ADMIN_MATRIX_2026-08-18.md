...
1. Release policy CURRENT resolve `SUBCATEGORY > CATEGORY > DEFAULT` por IDs estáveis e configuração publicada, com fallback fail-closed; o consumer legado sem classificação usa somente DEFAULT. Snapshot no checkout e relógio em `deliveredAt` continuam não implementados, e o lifecycle CURRENT do hold ainda exige `COMPLETED`/PAID/CONFIRMED`. Rating não tem efeito financeiro e somente Seller MAX poderá antecipar em capability futura. Requer E–G; evidência histórica permanece intacta.
