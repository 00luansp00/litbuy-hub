...
| SELLER-MAX-48H-QUALIFICATION | Confirmação em 48h corridas | Freeze | `Order` materializa snapshot v1 `PENDING`/`QUALIFIED`/`EXPIRED`; deadline deriva de `OrderDelivery.createdAt`; confirmação e expiry usam relógio PostgreSQL, lock do Order, eventos/outbox, replay idempotente e invariants PostgreSQL | `buyerConfirmedAt <= deliveredAt + 48h`; expira por venda | —; cálculo financeiro K permanece separado | delivery clock; MAX snapshot | AI_CANDIDATE | J | 2 | IMPLEMENTED | IMPLEMENTED |
| SELLER-MAX-RELEASE-CALC | Redução por blocos completos | Freeze | K v1 prospectiva persiste redução/target/effective no Order; G1/G2 preservam e validam o prazo base e consomem a boundary efetiva; calculator puro, trigger e testes cobrem blocos completos e nunca atrasar | `floor(days/7)*2`; `MIN(base, MAX(target, confirmation))`; nunca atrasar | — | qualification; release eligibility | AI_CANDIDATE | K | 2 | IMPLEMENTED | IMPLEMENTED |
| SELLER-MAX-STOCK-AUTOMATION | Target/consumo/pausa/restock | Freeze | CURRENT I3: snapshot MAX aciona auto-pause atômico no consumo pago; marker tipado distingue pausa manual; restock Seller aditivo, versionado e idempotente revalida publicação; audits, locks lifecycle/stock e testes PostgreSQL estão implementados | target de estoque, consumir por venda, pausar zero, restock válido | Implementado em I3 | MAX listing; inventory | AI_CANDIDATE | I3 | 5 | IMPLEMENTED | IMPLEMENTED |
...
| FEE-SNAPSHOT | Snapshot de rates/rules | Freeze; `FEE_SNAPSHOT.md`; CI #404 | `Order.feeSnapshotVersion` distingue legacy flat (`NULL`) de H2; `OrderFeeComponentSnapshot` relacional/versionado/imutável materializa `LISTING_TIER` na v1, `SELLER_MAX` na v2 e `BUYER_VIP` na v3, congelando policy/rule/publicVersion/qualifiers/category/party/formula/rate/base/amount/currency. Checkout cria Order + components na mesma transaction; replay não rerateia nem duplica; recognition valida snapshots sem policy atual/recalculation, falha fechado para ausência/corrupção e mantém legacy compatível | Congelar versão/regra/rates; histórico não recalculável | —; refund/reversal continua capability futura própria | listing tier | HYBRID_REVIEW_REQUIRED | H2 | 2 | IMPLEMENTED | IMPLEMENTED |
...
| BUYER-VIP-SELECTION | Sem Plano/Básico/Premium | Freeze; `BUYER_VIP_SELECTION.md` | Checkout exige escolha explícita `NONE`/`BASIC`/`PREMIUM`, inclui-a no request hash e fingerprint e congela snapshot v1 imutável; UI real começa sem seleção; legacy permanece NULL; sem fee ou entitlement | escolha explícita sem preselection paga | —; Q2/R/T permanecem separados | 1SKU | AI_CANDIDATE | Q1 | 4 | IMPLEMENTED | IMPLEMENTED |
| BUYER-VIP-FEE | 2,99%/4,99% Buyer | Freeze | PostgreSQL checkout/payment/recognition + UI | Base produto pós-descontos, Buyer charged | Fee policy/snapshot v3/ledger | VIP selection; refund contract | HYBRID_REVIEW_REQUIRED | Q2 | 4 | IMPLEMENTED | IMPLEMENTED |
...
| DISPUTE-PERSISTENT-CORE | Casos e histórico | Freeze; Recovery | `DisputeCase` e eventos append-only persistem lifecycle; state machine e triggers PostgreSQL validam transições; índice parcial garante um ativo/Order inclusive sob concorrência; testes reais cobrem FKs, história, imutabilidade e boundary financeira | vários históricos, máximo um ativo/Order, histórico imutável | —; integração com reporting/release permanece Y/Z | Order; auth | HYBRID_REVIEW_REQUIRED | X | 3 | IMPLEMENTED | IMPLEMENTED |
| BUYER-REPORT-PROBLEM | Backend real vitalício | Freeze; Post-chat; `BUYER_REPORT_PROBLEM.md` | API Buyer e UI de Order reais; ownership/IDOR, CSRF, lifecycle persistente, histórico/read model e reporting sem expiração testados | label, ownership/IDOR, lifetime reporting, eligibility | —; blocker financeiro continua exclusivamente em Z | dispute core | HYBRID_REVIEW_REQUIRED | Y | 3 | IMPLEMENTED | IMPLEMENTED |
| DISPUTE-PRE-RELEASE-BLOCK | Disputa bloqueia release | Freeze; Recovery; `DISPUTE_PRE_RELEASE_BLOCK.md` | G1 e G2 consultam `DisputeCase` sob row lock comum de Order; blockers persistentes e legado compõem fail-closed; batch filtra e `processOne()` revalida | blocker prevalece | — | dispute core; release | HYBRID_REVIEW_REQUIRED | Z | 3 | IMPLEMENTED | IMPLEMENTED |
| DISPUTE-POST-RELEASE-RECOVERY | Recovery após release | Recovery; Refund contract | NOT_PROVEN; boundaries documentais reconciliadas | pode gerar recovery/deficit decomposto; sem instant refund promise | AA0/AA0.1 existem; AA0.2 adiciona claim/FIFO e reserva inicial somente de AVAILABLE; deficit, fontes futuras e payout continuam pendentes | recovery core | HUMAN_PROD_REVIEW | AA | 7 | NOT_IMPLEMENTED | HUMAN_PROD_REVIEW |
...
| RECOVERY-QUEUE | FIFO por Seller | Recovery | AA0.2 prova foundation FIFO por `(executableAt, decisionId)`, Seller row lock e DB guard | executable-time FIFO, partial, nunca cross-Seller | Queue schema/locking/idempotency | deficit; final decision | HYBRID_REVIEW_REQUIRED | AB | 7 | NOT_IMPLEMENTED | IMPLEMENTED |
...
| RECOVERY-RESERVATION | Recursos reservados | Recovery | AA0.2 vincula allocation append-only ao posting AVAILABLE→RESERVED; fontes futuras não existem | Reservar até autorização | Ledger postings/read model | queue | HYBRID_REVIEW_REQUIRED | AC | 7 | PARTIAL | IMPLEMENTED |
