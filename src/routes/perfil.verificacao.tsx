...
function VerificacaoPage() {
  ...
  const advance = (from: VerificationStep["id"]) => {
    setSteps((prev) => prev.map((s) => (s.id === from ? { ...s, status: "completed" } : s)));
  ...
  const handleSubmitted = () => {
    ...
    toast.success("Verificação demonstrativa enviada para análise");
  };
    ...
    <>
      <div className="container-lit pt-4">
        <div className="rounded-xl border bg-card p-3 text-sm text-muted-foreground">
          A verificação de identidade continua demonstrativa; aprovação como vendedor não equivale
          ao selo Vendedor Verificado. Nenhum documento real, imagem, selfie ou biometria deve ser
          enviado nesta sprint.
        </div>
      </div>
            ...
            <div className="space-y-6 min-w-0">
              ...
              <section className="rounded-2xl border border-border bg-card p-5 shadow-card space-y-4">
                ...
                <div className="rounded-xl border border-border bg-surface/40 p-4">
                  {current === "basic" && (
                    <VerificationBasicStep onContinue={() => advance("basic")} />
                  )}
                  ...
                  {current === "selfie" && (
                    <VerificationSelfieStep onContinue={() => advance("selfie")} />
                  )}
            ...
            <aside className="space-y-6">
              <section className="rounded-2xl border border-border bg-card p-5 shadow-card">
                ...
                <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
                  <li>
                    • Selo <strong className="text-foreground">Vendedor Verificado</strong> na sua
                    loja.
                  </li>
