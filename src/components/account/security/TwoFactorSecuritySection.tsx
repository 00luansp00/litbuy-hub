...
export function TwoFactorSecuritySection({ smsAvailable }: { smsAvailable: boolean }) {
  ...
  const busy =
    ...
    inFlight.current ||
    reconcilingStatus;
  const ownTwoFactorBusy = busy || Boolean(challenge) || Boolean(disableChallenge);
  const otherTwoFactorBusy = ownTwoFactorBusy || showingRecoveryCodes;
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {statusReady && status.data?.enabled && status.data.method && (
          <TwoFactorMethodChange
            currentMethod={status.data.method}
            smsAvailable={smsAvailable}
            disabled={otherTwoFactorBusy}
            onExclusiveChange={setMethodChangeExclusive}
          />
        )}
        {statusReady && status.data?.enabled && !disableChallenge && !methodChangeExclusive && (
        ...
        {statusReady && disableChallenge && !methodChangeExclusive && (
