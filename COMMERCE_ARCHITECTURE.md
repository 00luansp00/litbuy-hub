...
Moeda inicial `BRL`; toda operação explicita moeda. Valores persistentes usam unidades mínimas inteiras e PostgreSQL `BIGINT` em centavos, nunca `float`/`double`. O backend converte o preço atual para centavos. A API usa `amountMinor` como número seguro ou string tipada, conforme limite contratual. Arredondamento é centralizado e testado.
...
- `PaymentStatus`: `NOT_CREATED`, `PENDING`, `PROCESSING`, `PAID`, `FAILED`, `EXPIRED`, `REFUND_PENDING`, `REFUNDED`, `CHARGEBACK`.
...
| Order       | ACTIVE → CANCELLED                      | operação autorizada        | política permite                | repõe só se aplicável    | estorno separado            | `order.cancelled`            | chave obrigatória  | compensatória                  |
...
| Payment     | REFUND_PENDING → PAID                   | sistema                    | refund falhou                   | nenhum                   | liberar reserva contábil    | `refund.failed`              | idem               | novo pedido de refund          |
